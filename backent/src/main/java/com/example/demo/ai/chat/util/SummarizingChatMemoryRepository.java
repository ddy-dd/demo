package com.example.demo.ai.chat.util;
...
public class SummarizingChatMemoryRepository implements ChatMemoryRepository {
    ...
    /**
     * 一次性工具名称集合 —— 这些工具的返回值只在"当前轮"生效，
     * 不应持久化到对话记忆中。AI 后续如需使用可重新调用。
     *
     * 当前包含：
     * - readSkill：返回完整的技能正文（数千字符），AI 读取后一次用完即可
     */
    private static final java.util.Set<String> ONE_TIME_TOOLS = java.util.Set.of("readSkill");

    /**
     * 从消息列表中过滤掉一次性工具的执行结果消息。
     * 使用 ToolResponseMessage.ToolResponse.name() 精确匹配工具名，
     * 不依赖消息正文内容，不涉及字符串模式匹配。
     */
    private List<Message> filterOneTimeToolResults(List<Message> messages) {
        return messages.stream()
                .filter(msg -> {
                    if (msg instanceof ToolResponseMessage trm) {
                        boolean isTransient = trm.getResponses().stream()
                                .anyMatch(r -> ONE_TIME_TOOLS.contains(r.name()));
                        if (isTransient) {
                            log.debug("过滤一次性工具执行结果: {}",
    ...
    public void saveAll(String conversationId, List<Message> messages) {
        ...
        try {
            ...
            // ── 过滤一次性工具执行结果（如 readSkill） ──────────────
            // readSkill 的返回值包含完整的技能正文（数千字符），
            // 但只在"当前推理轮"对 AI 有用。如果 AI 后续需要，可重新调用。
            // 过滤掉它们可以防止对话记忆被大量一次性内容撑爆。
            uniqueMessagesList = filterOneTimeToolResults(uniqueMessagesList);
            if (uniqueMessagesList.isEmpty()) {
                return;
            }
