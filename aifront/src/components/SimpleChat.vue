...
function handleSearchNavigate(uuid: string) {
  const el = messagesContainer.value?.querySelector(`[data-conv-uuid="${uuid}"]`)
  if (el) {
    const container = messagesContainer.value!
    const elRect = el.getBoundingClientRect()
    const containerRect = container.getBoundingClientRect()
    container.scrollTo({
      top: elRect.top - containerRect.top + container.scrollTop - 20,
      behavior: 'smooth',
    })
    ...
    <div class="chat-card">
      ...
      <div class="messages-area" ref="messagesContainer">
        ...
        <template v-for="conv in conversations" :key="conv.uuid">
          <div class="conversation-group" :class="'conv-status--' + conv.status" :data-conv-uuid="conv.uuid">
...
.scroll-to-bottom-btn {
  position: absolute;
  left: 50%;
  bottom: 10rem;          /* input-area 上方留足空间 */
...
/* ===== 定位闪烁高亮 ===== */
.conv-flash {
  animation: conv-flash 1.8s ease-out;
}
@keyframes conv-flash {
  0%   { background-color: rgba(255, 213, 79, 0.35); border-radius: 8px; }
  70%  { background-color: rgba(255, 213, 79, 0.15); border-radius: 8px; }
